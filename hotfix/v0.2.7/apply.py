...
def replace_method(s,signature_re,new_method,label):
    m=re.search(signature_re,s)
    if not m: raise SystemExit(f'PATCH_FAIL {label}: signature not found')
    start=m.start(); brace=s.find('{',m.start(),m.end()+2)
    if brace<0: raise SystemExit(f'PATCH_FAIL {label}: brace not found')
    depth=0; end=None
    ...
    if end is None: raise SystemExit(f'PATCH_FAIL {label}: body end not found')
    return s[:start]+new_method+s[end:]
...
# Native Android bars. The frozen source has setBars() and a bridge that ignored the dark argument.
...
new_bars=r'''    private void setBars(){ setBars(false); }

    private void setBars(boolean dark){
        getWindow().clearFlags(android.view.WindowManager.LayoutParams.FLAG_FULLSCREEN);
        getWindow().setStatusBarColor(Color.parseColor("#164BB7"));
        getWindow().setNavigationBarColor(Color.parseColor(dark ? "#0B1118" : "#F4F7FA"));
...
s=replace_method(s,r'\s*private\s+void\s+setBars\s*\(\s*\)\s*\{',new_bars,'setBars')
s=one(s,
'        @JavascriptInterface public void setSystemBars(boolean ignored){runOnUiThread(MainActivity.this::setBars);}',
'        @JavascriptInterface public void setSystemBars(boolean dark){runOnUiThread(()->MainActivity.this.setBars(dark));}',
'bridge-system-bars')
write(p,s)
...
s=one(s,
"  function setSystemBars(){try{AndroidApp.setSystemBars(false);}catch{}}",
r'''  function systemPrefersDark(){try{return window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches;}catch{return false;}}
...
"  try{window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change',()=>{if(state.theme==='system')applyTheme();});}catch{}\n  applyTheme();\n  render();scheduleReminders();setTimeout(()=>{if(state.lockMode!=='none')lockNow();},400);",
...
.theme-line>span{display:flex;flex-direction:column;align-items:flex-start}
